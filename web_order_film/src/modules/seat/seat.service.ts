...
export class SeatService {
  ...
  async getSeatsByCinema(cinemaId: number) {
    const seats = await this.seatReponsitory.find({ where: { cinemaId } });
    if (!seats.length) {
      throw new NotFoundException(`No seats found for cinema ID ${cinemaId}`);
    }
    return seats;
  }
